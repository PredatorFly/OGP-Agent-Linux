...
{
	my ($screen_id, $exec_cmd, $envVars, $server_exe, $home_path) = @_;
	...
	. "echo ". $SUDOPASSWD . " | sudo -S -p \"\" sh -c 'useradd ". $screen_id . "'\n"
	#. "echo -e ". $screen_id ."\n". $screen_id. " | passwd ". $screen_id . "\n"
	. "sudo chattr -i ./". $server_exe . "\n"
	. "sudo chown -R ". $screen_id . " ". $home_path ." \n"
	. "sudo chmod -R 770 * \n"
	. "sleep 1 \n"
	. "sudo chattr +i ./". $server_exe . "\n"
	. "sudo -u ". $screen_id . " bash -c '". $exec_cmd. "'\n"
	. "exit\n"
	. "}" . "\n"
	. "startServer" . "\n";
	...
	# Secure file
	secure_path_without_decrypt('chattr+i', $server_start_bashfile);
	#secure_path_without_decrypt('chattr+i', "*/hlds_run");
...
sub universal_start_without_decrypt
{
	...
	if($file_extension eq ".exe" or $file_extension eq ".bat")
	{
		...
		if(defined($Cfg::Preferences{ogp_autorestart_server}) &&  $Cfg::Preferences{ogp_autorestart_server} eq "1"){
			...
			$cli_bin = create_screen_cmd_loop($screen_id, $command, $envVars, $server_exe, $home_path);
		}else{
